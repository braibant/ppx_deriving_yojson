      ...
      begin
        ...
        | Some ({ ptyp_desc = Ptyp_constr ({ txt = lid }, args) } as manifest) ->
            ...
            ([ Str.module_ (Mb.mk (mknoloc mod_name) orig_mod) ],
             [ Vb.mk (pvar to_yojson_name) (polymorphize [%expr ([%e ser] : [%t typ] -> Yojson.Basic.json)]) ]
            )
        | Some _ ->
  ...
  | kind ->
      ...
      ([],
       [Vb.mk (pvar (Ppx_deriving.mangle_type_decl (`Suffix "to_yojson") type_decl))
          (polymorphize [%expr ([%e wrap_runtime serializer] : [%t typ] -> Yojson.Basic.json)])
       ])
  ...
  | kind ->
      ...
      ([],
       [Vb.mk (pvar (Ppx_deriving.mangle_type_decl (`Suffix "of_yojson") type_decl))
        (polymorphize [%expr ([%e wrap_runtime desurializer] : Yojson.Basic.json -> [%t error_or typ])])
       ])
